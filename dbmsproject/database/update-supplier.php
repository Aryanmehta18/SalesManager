...
$stmt->execute([$supplier_name, $supplier_location, $email, $supplier_id]); // Corrected line

$sql = "DELETE FROM productsuppliers WHERE supplier=?";
$stmt = $conn->prepare($sql);
$stmt->execute([$supplier_id]); // Corrected line
...
//   if($suppliers){
    ...
    $response = [
        'success' => false,
        'message' => "<strong>$supplier_name</strong> error adding to system: "
    ];
}
