...
$stmt->execute([$product_name, $description, $file_name_value, $pid]); // Corrected line

$sql = "DELETE FROM productsuppliers WHERE product=?";
$stmt = $conn->prepare($sql);
$stmt->execute([$pid]); // Corrected line
...
//   if($suppliers){
    ...
    $response = [
        'success' => false,
        'message' => "<strong>$product_name</strong> error adding to system: "
    ];
}
