...
if($type === 'supplier'){
    $stmt = $conn->prepare("
        SELECT 
            suppliers.id as sid, 
            suppliers.created_at as 'created at', 
            users.first_name, 
            users.last_name, 
    ...
    foreach($suppliers as $supplier){
        ...
        unset($supplier['first_name'], $supplier['last_name']); // Remove this line
        if($is_header){
...
if($type === 'purchase_orders'){
    ...
    $is_header = true;
    foreach($pos as $batch => $order_products_batch){
        foreach($order_products_batch as $order_product){
            $order_product['created_by'] = $order_product['first_name'] . ' ' . $order_product['last_name'];
            unset($order_product['first_name'], $order_product['last_name']); // Remove this line
...
if($type === 'delivery'){
    ...
    foreach($delivery_by_batch as $deliveries){
        foreach($deliveries as $delivery){
            ...
            unset($delivery['first_name'], $delivery['last_name']); // Remove this line
